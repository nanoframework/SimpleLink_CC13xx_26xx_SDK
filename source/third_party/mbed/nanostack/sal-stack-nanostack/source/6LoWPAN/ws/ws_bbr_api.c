...
//NVM file name
const char *BBR_INFO_FILE = "pae_bbr_info";
...
static uint8_t static_dodag_prefix[8] = {0x20, 0x20, 0xab, 0xcd};
static uint8_t static_dodag_id_prefix[8] = {0x20, 0x20, 0xab, 0xcd};
...
static uint32_t bbr_delay_timer = 2; // initial delay.
#else
...
static void ws_bbr_rpl_status_check(protocol_interface_info_entry_t *cur)
{
    ...
    if ((configuration & BBR_ULA_C) == 0 && memcmp(global_prefix, ADDR_UNSPECIFIED, 8) == 0) {
        ...
        if (NULL != ws_bbr_bb_static_prefix_get(NULL)) {
            ...
            prefix_wait_time = 40;
#else
            prefix_wait_time = BBR_BACKUP_ULA_DELAY;
#endif
        }
