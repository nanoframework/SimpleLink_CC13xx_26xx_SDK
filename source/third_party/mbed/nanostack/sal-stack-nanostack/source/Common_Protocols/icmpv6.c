...
#ifdef WISUN_NCP_ENABLE
    /* Send Packets destined for address outside RPL network to BR back-end host.
     * BR Host IP Stack is expected to handle such packets appropriately */
    if ((type == ICMPV6_TYPE_ERROR_DESTINATION_UNREACH) &&
        (addr_ipv6_scope(buf->dst_sa.address, buf->interface) > IPV6_SCOPE_REALM_LOCAL))
    {
...
{
//#ifndef TI_WISUN_FAN_OPT_ICMPV6
