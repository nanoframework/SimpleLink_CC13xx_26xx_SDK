...
//! @brief Function to read struct bt_mesh_hb_sub_raw
static void read_bt_mesh_hb_sub_raw_struct(erpc::Codec * codec, bt_mesh_hb_sub_raw * data);

//! @brief Function to read struct bt_mesh_cfg_srv_raw
static void read_bt_mesh_cfg_srv_raw_struct(erpc::Codec * codec, bt_mesh_cfg_srv_raw * data);

//! @brief Function to read struct bt_mesh_hb_pub_raw
static void read_bt_mesh_hb_pub_raw_struct(erpc::Codec * codec, bt_mesh_hb_pub_raw * data);
...
static void read_bt_mesh_model_cb_raw_struct(erpc::Codec * codec, bt_mesh_model_cb_raw * data)
{
    ...
    if (!isNull)
    {
        codec->readCallback((funPtr)(settings_commit_cb), (funPtr *)(&data->settings_commit));
    }
    else
    {
        data->settings_commit = NULL;
    }
...
// Read struct bt_mesh_hb_sub_raw function implementation
static void read_bt_mesh_hb_sub_raw_struct(erpc::Codec * codec, bt_mesh_hb_sub_raw * data)
{
    bool isNull;
    codec->read(&data->expiry);
    ...
    if (!isNull)
    {
        codec->readCallback((funPtr)(hb_sub_cb), (funPtr *)(&data->func));
    }
    else
    {
        data->func = NULL;
...
// Read struct bt_mesh_cfg_srv_raw function implementation
static void read_bt_mesh_cfg_srv_raw_struct(erpc::Codec * codec, bt_mesh_cfg_srv_raw * data)
{
    bool isNull;
    codec->read(&data->model_placeholder);

    codec->read(&data->net_transmit);

    codec->read(&data->relay);

    codec->read(&data->relay_retransmit);

    codec->read(&data->beacon);

    codec->read(&data->gatt_proxy);

    codec->read(&data->frnd);

    codec->read(&data->default_ttl);

    codec->readNullFlag(&isNull);
    if (!isNull)
    {
        data->hb_pub = (bt_mesh_hb_pub_raw *) erpc_malloc(sizeof(bt_mesh_hb_pub_raw));
        if (data->hb_pub == NULL)
        {
            codec->updateStatus(kErpcStatus_MemoryError);
        }
        read_bt_mesh_hb_pub_raw_struct(codec, &(*data->hb_pub));
    }
    else
    {
        data->hb_pub = NULL;
    ...
    if (!isNull)
    {
        data->hb_sub = (bt_mesh_hb_sub_raw *) erpc_malloc(sizeof(bt_mesh_hb_sub_raw));
        if (data->hb_sub == NULL)
        {
            codec->updateStatus(kErpcStatus_MemoryError);
        }
        read_bt_mesh_hb_sub_raw_struct(codec, &(*data->hb_sub));
    }
    else
    {
        data->hb_sub = NULL;
...
// Read struct bt_mesh_hb_pub_raw function implementation
static void read_bt_mesh_hb_pub_raw_struct(erpc::Codec * codec, bt_mesh_hb_pub_raw * data)
{
    codec->read(&data->timer_placeholder);
...
//! @brief Function to free space allocated inside struct bt_mesh_cfg_srv_raw
static void free_bt_mesh_cfg_srv_raw_struct(bt_mesh_cfg_srv_raw * data);
...
// Free space allocated inside struct bt_mesh_cfg_srv_raw function implementation
static void free_bt_mesh_cfg_srv_raw_struct(bt_mesh_cfg_srv_raw * data)
{
    if (data->hb_pub) // this is a nullable member
    {

    }
    if (data->hb_pub)
    {
        erpc_free(data->hb_pub);
    }

    if (data->hb_sub) // this is a nullable member
    {

    }
    if (data->hb_sub)
    {
        erpc_free(data->hb_sub);
    }
}
...
#ifndef ERPC_REMOVE__bt_mesh_lpn_set_cb
        case kBLEmesh_api_bt_mesh_lpn_set_cb_id:
            return bt_mesh_lpn_set_cb_shim(codec, messageFactory, sequence);
#endif
...
erpc_status_t BLEmesh_api_service::bt_mesh_cfg_srv_raw_init_shim(Codec * codec, MessageBufferFactory *messageFactory, uint32_t sequence)
{
    ...
    uint16_t elem_index;
    bt_mesh_cfg_srv_raw *cfg_srv = NULL;
    cfg_srv = (bt_mesh_cfg_srv_raw *) erpc_malloc(sizeof(bt_mesh_cfg_srv_raw));
    if (cfg_srv == NULL)
    {
        codec->updateStatus(kErpcStatus_MemoryError);
    }
    ...
    read_bt_mesh_cfg_srv_raw_struct(codec, cfg_srv);
    ...
    if (!err)
    {
        ...
        nestingDetection = true;
#endif
        result = bt_mesh_cfg_srv_raw_init(elem_index, cfg_srv);
#if ERPC_NESTED_CALLS_DETECTION
        nestingDetection = false;
#endif
    ...
    if (cfg_srv)
    {
        free_bt_mesh_cfg_srv_raw_struct(cfg_srv);
    }
    if (cfg_srv)
    {
        erpc_free(cfg_srv);
...
// Server shim for bt_mesh_lpn_set_cb of BLEmesh_api interface.
#ifndef ERPC_REMOVE__bt_mesh_lpn_set_cb
erpc_status_t BLEmesh_api_service::bt_mesh_lpn_set_cb_shim(Codec * codec, MessageBufferFactory *messageFactory, uint32_t sequence)
{
    erpc_status_t err = kErpcStatus_Success;
    ...
    if (!isNull)
    {
        codec->readCallback((funPtr)(lpn_set_cb), (funPtr *)(&cb));
    }
    else
    {
        cb = NULL;
    ...
    if (!err)
    {
        ...
        // Build response message.
        codec->startWriteMessage(kReplyMessage, kBLEmesh_api_service_id, kBLEmesh_api_bt_mesh_lpn_set_cb_id, sequence);
