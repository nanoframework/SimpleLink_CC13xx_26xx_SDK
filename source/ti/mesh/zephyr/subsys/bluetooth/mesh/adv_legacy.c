...
#define ADV_START_TIME_MS  5
#define MAX_ADV_DELAY      10
...
static inline void adv_send(struct net_buf *buf)
{
    ...
    int advTransInt   = BT_MESH_TRANSMIT_INT(BT_MESH_ADV(buf)->xmit);
    int advTransCount = BT_MESH_TRANSMIT_COUNT(BT_MESH_ADV(buf)->xmit);

    // WA: When BLE Simple Peripheral is also present in the system, need to add
    // more time for the mesh adv packet, since the SP adv might preempt the Mesh
    // Adv packet
#ifdef ADD_SIMPLE_PERIPHERAL
    advTransCount += 2;
#endif

    adv_int = MAX(adv_int_min, advTransInt);
    duration = advTransCount * (adv_int + MAX_ADV_DELAY) + ADV_START_TIME_MS;
