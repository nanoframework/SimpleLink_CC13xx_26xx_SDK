		       ...
		       struct net_buf_simple *buf)
{
	 ...
	 * send it later.
	 */
	if (bt_mesh_model_send(model, ctx, &msg, &node_reset_cb, NULL)) {
...
#pragma diag_group_id 112
#elif __IAR_SYSTEMS_ICC__
#pragma diag_suppress=Pe111 //Suppress warning "statement is unreachable"
#else
...
ZEPHYR__PACKED(struct) hb_pub_param {
	u16_t dst;
	u8_t  count_log;
	u8_t  period_log;
	u8_t  ttl;
...
static void hb_sub_send_status(struct bt_mesh_model *model,
			       struct bt_mesh_msg_ctx *ctx, u8_t status, bool is_sub_get_addr_unasigned)
{
	...
	// According to mesh spec, if we receive Config Heartbeat Subscription Get
	// and source or destination addresses state is unassigned,
	// the response Heartbeat Subscription Status should contain the following values (all 0).
	// But the actual values (called "state" in the spec), should not be changed
	if(is_sub_get_addr_unasigned)
	{
        net_buf_simple_add_le16(&msg, BT_MESH_ADDR_UNASSIGNED); // Source address
        net_buf_simple_add_le16(&msg, BT_MESH_ADDR_UNASSIGNED); // Destination address
        net_buf_simple_add_u8(&msg, 0x00); // Heartbeat Period
        net_buf_simple_add_u8(&msg, 0x00); // CountLog
        net_buf_simple_add_u8(&msg, 0x00); // Minimum hops
        net_buf_simple_add_u8(&msg, 0x00); // Maximum hops
	}
	else
	{
			      ...
			      struct net_buf_simple *buf)
{
	...
	{
	    hb_sub_send_status(model, ctx, STATUS_SUCCESS,TRUE);
	}
	else
	{
	    hb_sub_send_status(model, ctx, STATUS_SUCCESS,FALSE);
	}
