...
// When using IAR, seg_rx is initialized at bt_mesh_trans_init
static struct seg_rx seg_rx[CONFIG_BT_MESH_RX_SEG_MSG_COUNT];
...
static u8_t seg_rx_buf_data[(CONFIG_BT_MESH_RX_SEG_MSG_COUNT *
				      CONFIG_BT_MESH_RX_SDU_MAX)] __attribute__ ((section (".noinit")));
#else
...
void bt_mesh_trans_init(void)
{
  ...
  for (i = 0; i < ARRAY_SIZE(seg_rx); i++)
  {
    seg_rx[i].buf.size = CONFIG_BT_MESH_RX_SDU_MAX;
  }
#endif /* ifdef __IAR_SYSTEMS_ICC__ */
...
#pragma diag_group_id 187
#elif defined(__IAR_SYSTEMS_ICC__)
#pragma diag_suppress=Pe185 // Suppress warning "dynamic initialization in unreachable code"
#else
