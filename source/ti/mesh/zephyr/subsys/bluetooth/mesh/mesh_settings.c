...
ZEPHYR__PACKED(struct) net_val {
	uint16_t primary_addr;
	uint8_t  dev_key[16];
};

/* Sequence number storage */
ZEPHYR__PACKED(struct) seq_val {
	uint8_t val[3];
};
...
ZEPHYR__PACKED(struct) net_key_val {
	uint8_t kr_flag:1,
	     kr_phase:7;
	uint8_t val[2][16];
};
...
ZEPHYR__PACKED(struct) app_key_val {
	uint16_t net_idx;
	bool  updated;
	uint8_t  val[2][16];
};
...
ZEPHYR__PACKED(struct) va_val {
	uint16_t ref;
	uint16_t addr;
	uint8_t uuid[16];
};

ZEPHYR__PACKED(struct) cdb_net_val {
	uint32_t iv_index;
	bool  iv_update;
};

/* Node storage information */
ZEPHYR__PACKED(struct) node_val {
	uint16_t net_idx;
	uint8_t  num_elem;
	uint8_t  flags;
...
const struct settings_handler_static settings_handler_bt_mesh = {
    .name = "bt/mesh",
    .h_get = 0,
    .h_set = mesh_set,
...
#endif /* defined(CONFIG_BT_MESH_CDB) */
}
					       ...
					       struct node_update **free_slot)
{
	...
	*free_slot = NULL;
	...
	return match;
#else /* defined(CONFIG_BT_MESH_CDB) */
    return NULL;
#endif /* defined(CONFIG_BT_MESH_CDB) */
}
