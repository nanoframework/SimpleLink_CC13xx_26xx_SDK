...
/* Unaligned access */
#define UNALIGNED_GET(p) (uintptr_t)(p)
...
/* Defining packed macros */
#define ZEPHYR__PACKED(__TYPE__) __packed __TYPE__
#define ZEPHYR__TYPEDEF_PACKED(__TYPE__) __packed typedef __TYPE__
...
static inline unsigned int popcount(unsigned int v)
{
    ...
    return ((v + (v >> 4u) & 0xF0F0F0F) * 0x1010101u) >> 24u; // count
}
...
/* From ffs.h from TI Arm compiler */
static inline int __builtin_ffs(int i)
{
   int c = 1;
...
// Adding missing errno.h definitions
#define E2BIG          7
#define EBADMSG        77
#define ECANCELED      47
#define ECHILD         10
