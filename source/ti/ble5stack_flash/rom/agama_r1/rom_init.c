...
extern uint8 llSetExtendedAdvReport(aeExtAdvRptEvt_t *extAdvRpt, uint8 *pPkt, uint16 evtType,uint8 extHdrFlgs, uint8 pHdr, uint8 dataLen, uint8 **pSyncInfo,uint8 *secPhy);
...
uint8 MAP_LL_ChanMapUpdate( uint8 *chanMap )
{
#if defined(CTRL_CONFIG) && (CTRL_CONFIG & INIT_CFG)
  return LL_ChanMapUpdate(chanMap);
#else
  return 0;
#endif
}
