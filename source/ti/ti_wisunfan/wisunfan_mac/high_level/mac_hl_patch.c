...
#define ED_SCAN_RESOLUTION_CLOCK_TICKS  (MAC_ED_SCAN_RESOLUTION * (1000 / Clock_tickPeriod))
...
#if 0 // Need to add check for key Id & framecounter
      if(securityLevel == 6) /* WISUN mandates security level of 6 */
      {
          if ((pMsg->internal.frameCntr == MAC_MAX_FRAME_COUNTER ) || (pMsg->internal.frameCntr < (uint8 *)pDeviceDescriptor->frameCounter ))
          {
            pMsg->hdr.status = MAC_COUNTER_ERROR;
#ifdef FH_HOP_DEBUG
            FH_Dbg.numFramecounterErr++;
#endif
...
#endif
#ifndef FEATURE_WISUN_SUPPORT
#warning "MAC Incoming Frame Counter Handling Needs to be fixed"
  /* Check the frame counter and compare with the frame counter in device descriptor */
  if ((pMsg->internal.frameCntr == MAC_MAX_FRAME_COUNTER ) || (pMsg->internal.frameCntr < pDeviceDescriptor->frameCounter ))
  {
    pMsg->hdr.status = MAC_COUNTER_ERROR;
    return (MAC_COUNTER_ERROR);
  }
  ...
  *(--pBuf->msdu.p) = MAC_HEADER_IE_TERM_SECOND_BYTE;
  ...
  for (i = 0; i < MAC_HEADER_IE_MAX; i++ )
  {
    ...
    if ( (MAC_HEADER_IE_WISUN == pHeaderIes[i].elementId) && (pHeaderIes[i].subIdBitMap) )
    {
      ...
      if ( 0 == MAC_FH_GENERATE_IE_FN(pBuf->msdu.p, pHeaderIes[i].subIdBitMap, pBuf, pIeInfo))
      {
        status = MAC_NO_DATA;
        return status;
      }
...
uint8 MAC_McpsDataReq(ApiMac_mcpsDataReq_t* pData)
{
       ...
       (macCfg.txDataMax > macData.indirectCount)))
  {
        ...
        if((switchToEDFE > 0) && (pData->includeFhIEs & MAC_FH_MPX_IE) && (pData->dstAddr.addrMode != SADDR_MODE_NONE))
        {
