/**************************************************************************************************
  Filename:       mac_rx.c
  Revised:        $Date: 2015-03-20 14:00:36 -0700 (Fri, 20 Mar 2015) $
  Revision:       $Revision: 43240 $
...
MAC_INTERNAL_API void macRxHaltCleanup(void)
{
  ...
  else
  {
    if (macRxOnFlag)
    {
      ...
      macCheckCommandDone(&macRxEdScan.rxCmd.rfOpCmd);

      /* Wait till all FG commands are done */
      macCheckCommandDone(&macCsmaCaCmd.rfOpCmd);
      macCheckCommandDone(&macTxCmd.rfOpCmd);
      macCheckCommandDone(&macRxAckCmd.rfOpCmd);
    }
...
inline static void rxFcsIsr(void)
{
  ...
  */
  ...
  if ( macTxAckPending == MAC_TX_ACK_PENDING )
  {
    DBG_PRINT0(DBGSYS, "macBackoffTimerCancelTriggerWakeup()");
    macTxAckPending = MAC_TX_ACK_NOT_PENDING;
    macBackoffTimerCancelTriggerWakeup();
  }
