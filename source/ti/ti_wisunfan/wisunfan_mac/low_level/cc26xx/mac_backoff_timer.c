...
/* Use LED4 for sleep debug with MODULE_CC26XX_7X7 */
#ifdef DEBUG_SLEEP_LED
#define DEBUG_SLEEP_LED_ON()  HWREG(GPIO_BASE+GPIO_O_DOUTSET31_0) = (1UL<<IOID_6)
#define DEBUG_SLEEP_LED_OFF() HWREG(GPIO_BASE+GPIO_O_DOUTCLR31_0) = (1UL<<IOID_6)
#else
...
/**************************************************************************************************
 * @fn          macBackoffTimerCancelTriggerWakeup
 *
 * @brief       Cancels the trigger to stop an unnecessary wakeup. This is
 *              achieved by starting a short timeout on TI-RTOS clock.
 *              If you want to stop and restart the clock with an immediate
 *              timeout, the very quickest timeout could be ~60+ usec later.
 *              (The actual timeout will vary because of synchronization with
 *              the next 32KHz clock edge.) This number comes from the RTC’s
 *              dead window for scheduling new events. A new quick timeout seems
 *              the easiest way to force a quick reschedule of the timer interrupt,
 *              and skip the timer interrupt that was scheduled originally.
 *
 * @param       none
 *
 * @return      none
 **************************************************************************************************
 */
#define MAC_QUICKEST_TIMEOUT_US 80
...
#ifdef USE_ICALL
    ICall_setTimer((uint_fast32_t)(MAC_QUICKEST_TIMEOUT_US / osal_tickperiod),
                   macBackoffTimerICallTimerCback,
                   NULL, &macBackoffTimerICallTimerID);
#endif /* USE_ICALL */
#ifdef OSAL_PORT2TIRTOS
    Clock_stop(macBackoffWakeupClock);
    Clock_setTimeout(macBackoffWakeupClock, (UInt32)(MAC_QUICKEST_TIMEOUT_US / Clock_tickPeriod));
    Clock_start(macBackoffWakeupClock);
#endif /* OSAL_PORT2TIRTOS */
